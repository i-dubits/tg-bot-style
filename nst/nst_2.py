# -*- coding: utf-8 -*-
"""
Created on Mon Jan 25 18:57:02 2021
...
class Image_proc():
    ...
    def __init__(self, imsize, device):
    ...
    async def my_imsave(self, tensor, filename='output.png'):
...
class Run_NST():
        ...
        def __init__(self, style_img, content_img, device):
            ...
            self.cnn.load_state_dict(torch.load(config.nst_state_dict))
            self.cnn = self.cnn.features.to(device).eval()
        ...
        async def run_style_transfer(self, num_steps=300, style_weight=1000000, content_weight=1):
